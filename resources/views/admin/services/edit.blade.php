...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <form action="{{ route('admin.services.update', $service) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')

            <x-admin.input label="Service Title" name="title" :value="$service->title" placeholder="Web Development, SEO, UI Design..." required="true" :error="$errors->first('title')" />

            <x-admin.input label="Icon Class" name="icon" :value="$service->icon" placeholder="ri-code-line, ri-layout-line..." :error="$errors->first('icon')" />
