...
@section('page-subtitle', 'Update details for: ' . $project->title)
            ...
            <form action="{{ route('admin.projects.update', $project) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <x-input label="Tech Stack (comma separated)" name="tech_stack" value="{{ is_array($project->tech_stack) ? implode(', ', $project->tech_stack) : $project->tech_stack }}" placeholder="Laravel, Tailwind, Vue.js..." :error="$errors->first('tech_stack')" />

                        <div class="grid grid-cols-2 gap-4">
                            <x-input label="Sort Order" name="sort_order" type="number" value="{{ $project->sort_order }}" required="true" :error="$errors->first('sort_order')" />
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-3 bg-dark-900 border border-dark-600 rounded-xl px-4 py-2.5">
                                    <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $project->is_featured) ? 'checked' : '' }} class="w-5 h-5 rounded bg-dark-800 border-dark-600 text-neon-500 focus:ring-neon-500/20 focus:ring-offset-dark-950">
                ...
                <div class="space-y-6 pt-4">
                    ...
                    <div class="grid md:grid-cols-2 gap-8 items-start">
                        <div class="space-y-4">
                            <label class="block text-xs font-bold text-dark-400 uppercase tracking-widest px-1">Upload New Images</label>
                        ...
                        @if(!empty($project->images))
                            <div class="space-y-4">
                                <label class="block text-xs font-bold text-dark-400 uppercase tracking-widest px-1">Current Images (Select to delete)</label>
                                <div class="grid grid-cols-2 gap-3">
                                    @foreach($project->images as $index => $image)
                                        <div class="relative aspect-video rounded-xl overflow-hidden border border-dark-600 group">
                                            ...
                                            <div class="absolute top-2 right-2">
                                                <input type="checkbox" name="delete_images[]" value="{{ $image }}" class="w-5 h-5 rounded bg-dark-950/80 border-red-500/50 text-red-500 focus:ring-red-500/20">
                                            </div>
                                            <div class="absolute inset-0 bg-red-500/10 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
                                        </div>
                                    @endforeach
                                </div>
                                <p class="text-[10px] font-black uppercase tracking-widest text-dark-600 italic">Check the box on an image to remove it upon saving.</p>
                            </div>
                        @endif
                    </div>
                </div>
