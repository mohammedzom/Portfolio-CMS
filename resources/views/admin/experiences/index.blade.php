...
<div class="sm:flex sm:items-center justify-between mb-8">
    ...
    <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none flex gap-3">
        ...
        <a href="{{ route('admin.experience.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-center text-sm font-bold text-white shadow-sm hover:bg-indigo-500 transition-all">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    <th scope="col" class="py-4 pl-6 pr-3 text-left text-xs font-black uppercase tracking-widest text-slate-500">Role & Company</th>
                    <th scope="col" class="px-3 py-4 text-left text-xs font-black uppercase tracking-widest text-slate-500">Duration</th>
                    ...
                    <tr class="hover:bg-slate-50 transition-colors group">
                        ...
                        <td class="whitespace-nowrap px-3 py-5">
                            ...
                            @elseif($experience->is_current)
                                <span class="inline-flex items-center rounded-lg bg-emerald-50 px-2 py-1 text-[10px] font-black text-emerald-600 border border-emerald-100 uppercase tracking-widest">
                                    Current
                                </span>
                            @else
                        ...
                        <td class="relative whitespace-nowrap py-5 pl-3 pr-6 text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                @else
                                    <a href="{{ route('admin.experience.edit', $experience) }}" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Edit">
                                        <i class="ri-pencil-line text-lg"></i>
                                    </a>
                                    <form action="{{ route('admin.experience.destroy', $experience) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    <tr>
                        <td colspan="4" class="py-20 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <a href="{{ route('admin.experience.create') }}" class="text-sm font-bold text-indigo-600 hover:text-indigo-700">Add your first experience</a>
                            </div>
                        </td>
                    </tr>
                @endforelse
