...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <form action="{{ route('admin.experience.store') }}" method="POST" class="p-8 space-y-6">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <x-admin.input label="Job Title" name="title" placeholder="Senior Web Developer..." required="true" :error="$errors->first('title')" />
                <x-admin.input label="Company Name" name="company" placeholder="Google, Freelance, etc..." required="true" :error="$errors->first('company')" />
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <x-admin.input label="Start Date" name="start_date" type="date" required="true" :error="$errors->first('start_date')" />
                <x-admin.input label="End Date" name="end_date" type="date" :error="$errors->first('end_date')" />
            </div>
            ...
            <label class="flex items-center gap-3 p-4 rounded-xl border border-slate-100 bg-slate-50 cursor-pointer hover:bg-indigo-50 transition-colors">
                <input type="checkbox" name="is_current" value="1" {{ old('is_current') ? 'checked' : '' }} class="h-5 w-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
