<div class="flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4">
    <div class="flex h-16 shrink-0 items-center gap-2">
        ...
        <span class="text-xl font-black tracking-tight text-slate-900">Admin<span
                class="text-indigo-600">Panel</span></span>
    </div>
                    ...
                    @foreach ($navItems as $item)
                        <li>
                            <a href="{{ route($item['route']) }}"
                                class="{{ request()->routeIs($item['route'] . '*') ? 'bg-indigo-50 text-indigo-600' : 'text-slate-700 hover:text-indigo-600 hover:bg-slate-50' }} group flex gap-x-3 rounded-xl p-3 text-sm font-semibold leading-6 transition-all duration-200">
                                <i
                                    class="{{ $item['icon'] }} text-lg {{ request()->routeIs($item['route'] . '*') ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600' }}"></i>
                                {{ $item['label'] }}
                            </a>
                        </li>
                    @endforeach
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <p class="text-2xl font-black text-indigo-400">
                                {{ \App\Models\Message::where('is_read', false)->count() }}</p>
